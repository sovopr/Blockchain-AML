...
class GraphRenderer {
    constructor(container) {
        ...
        // Soft Pastel Colors
        this.colors = {
            low: '#a8d5ba',      // Soft mint green
            medium: '#f4c19f',   // Soft peach
            high: '#f3a9a9'      // Soft coral
        };
    ...
    renderLevel3(data, onWalletClick) {
        ...
        const links = this.g.selectAll('.edge')
            ...
            .attr('class', 'edge')
            .attr('stroke', '#cbd5e0')
            .attr('stroke-opacity', d => 0.2 + d.recency * 0.3)
            .attr('stroke-width', d => Math.sqrt(d.amount) / 2)
        ...
        const nodeElements = this.g.selectAll('.wallet-node')
            ...
            .attr('fill', d => this.colors[d.riskLevel])
            .attr('stroke', d => d.id === centerNode.id ? '#7c8db5' : '#fff')
            .attr('stroke-width', d => d.id === centerNode.id ? 3 : 2)
            .attr('cursor', 'pointer')
            .on('click', (event, d) => {
                event.stopPropagation();
                if (onWalletClick) onWalletClick(d);
